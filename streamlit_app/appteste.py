...
def tab_desenv_d7(df_raw: pd.DataFrame):
    ...
    # Exibição: um painel (facet) por ADVP (linhas verticais empilhadas). Em cada painel:
    # X = data (últimos 7 dias), Y = % de participação (0-100%), uma linha por empresa (123MILHAS, MAXMILHAS, FLIPMILHAS).
    chart_data = wins_full.copy()
    # Garantir que DT seja temporal e ordenado
    chart_data["DT"] = pd.to_datetime(chart_data["DT"]) 
    chart_data["DT_LABEL"] = chart_data["DT"].dt.strftime("%d/%m")
    chart_data["DT"] = pd.to_datetime(chart_data["DT"].dt.date)
    chart_data["DT"] = chart_data["DT"].dt.normalize()

    chart_data["ADVP_LABEL"] = chart_data["ADVP_CANON_NUM"].astype(int).astype(str)
    # Categoricals para ordenar corretamente
    chart_data["DT"] = pd.Categorical(chart_data["DT"], categories=all_dates, ordered=True)
    ...
    line = alt.Chart(chart_data).mark_line(point=True).encode(
        x=alt.X("DT:T", title="Data", axis=alt.Axis(format="%d/%m", labelAngle=0)),
