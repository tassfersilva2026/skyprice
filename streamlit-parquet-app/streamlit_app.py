...
def tab1_painel(df_raw: pd.DataFrame):
    ...
    if not W.empty:
        # Pega a lista de agências com vitórias em 1º lugar (baseado no DF original)
        top_agencies = W["R1"].value_counts().index.tolist()
        
        # Cria a lista final removendo apenas a categoria "SEM OFERTAS"
        agencies_to_show = [
            agency for agency in top_agencies 
            if agency != "SEM OFERTAS"
        ]
