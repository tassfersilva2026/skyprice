...
# ---- Estilo para listas verticais de cards (Painel por Cia) ----
CARDS_STACK_CSS = """
<style>
...
def _init_filter_state(df_raw: pd.DataFrame):
    ...
    st.session_state["flt"] = {
        ...
        "advp": [], "trechos": [], "hh": [], "cia": []  # filtro global de Cia
    }
...
def tab1_painel(df_raw: pd.DataFrame):
    ...
    st.caption("Ranking das agências por companhia — cards empilhados por coluna.")
    st.markdown(CARDS_STACK_CSS, unsafe_allow_html=True)

    if "CIA" not in df.columns:
        st.info("Coluna 'CIA' não encontrada nos dados filtrados.")
        return
    ...
    def render_por_cia(container, df_in: pd.DataFrame, cia_name: str):
        with container:
            ...
            sub = df_in[df_in["CIA"].astype(str).str.upper() == cia_name]
            if sub.empty:
            ...
            }).replace({"GRURO 123":"GRUPO 123"})  # proteção caso digitem errado
            ...
            cards_local = []
            for tgt in targets_sorted_local:
                p1, p2, p3 = pct_target(tgt)
                cards_local.append(card_html(tgt, p1, p2, p3))
...
def tab2_top3_agencias(df_raw: pd.DataFrame):
    ...
    def pct_diff(base, other):
        if _np.isnan(base) or base == 0 or _np.isnan(other):
            return _np.nan
    ...
    sty2 = (
        t2.style.format(fmt_map_t2, na_rep="-", decimal=",", thousands=".")
          .set_table_styles([{"selector":"tbody td, th","props":[("border","1px solid #EEE")]}])
          .set_properties(**{"background-color":"#ffffff","color":"#111111"})
    )
    ...
    sty3 = (
        t3.style.format(fmt_map_t3, na_rep="-", decimal=",", thousands=".")
          .set_table_styles([{"selector":"tbody td, th","props":[("border","1px solid #EEE")]}])
          .set_properties(**{"background-color":"#ffffff","color":"#111111"})
    )
    ...
    def pct_vs_base(base, x):
        if _np.isnan(base) or base == 0 or _np.isnan(x):
            return _np.nan
    ...
    sty4 = (
        t4.style.format(fmt_map_t4, na_rep="-", decimal=",", thousands=".")
          .set_table_styles([{"selector":"tbody td, th","props":[("border","1px solid #EEE")]}])
          .set_properties(**{"background-color":"#ffffff","color":"#111111"})
    )
...
def tab4_melhor_preco_por_periodo(df_raw: pd.DataFrame):
    ...
    def parse_price_cell(x) -> float | None:
        ...
        try:
            ...
            s = str(x)
            m = re.search(r"(?:R\$\s*)?(\d{1,3}(?:\.\d{3})*,\d{2}|\d+,\d{2}|\d{1,3}(?:\.\d{3})+|\d+)", s)
    ...
    price_series = (df["PRECO"] if "PRECO" in df.columns else df.get("VALOR", pd.Series([np.nan]*len(df))))
