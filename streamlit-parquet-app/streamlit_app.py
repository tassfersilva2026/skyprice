...
# === Parser robusto pra hora em texto -> "HH:MM:SS" ===
def parse_hora_text(val) -> str | None:
    s = str(val).strip()
    if s == "" or s.lower() in {"nan", "none", "null"}:
        return None

    # 1) tenta interpretar como data/hora completa primeiro
    try:
    ...
    # 2) tenta por dígitos puros (HH, HHMM, HHMMSS etc.)
    ...
    try:
        ...
        elif len(digs) in (1, 2):        # H ou HH
            h, m, sec = int(digs), 0, 0
    except Exception:
        pass

    # 3) tenta pelos dois-pontos (H:M ou H:M:S)
    if h is None:
