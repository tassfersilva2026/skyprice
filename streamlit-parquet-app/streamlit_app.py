...
def load_base(path: Path) -> pd.DataFrame:
    ...
    colmap = {
        0: "IDPESQUISA", 1: "CIA", 2: "HORA_BUSCA", 3: "HORA_PARTIDA",
        4: "HORA_CHEGADA", 5: "TIPO_VOO", 6: "DATA_EMBARQUE",
        7: "DATAHORA_BUSCA", 8: "AGENCIA_COMP", 9: "PRECO",
...
def tab2_top3_agencias(df_raw: pd.DataFrame):
    ...
    def style_heatmap_discrete(styler: _pd.io.formats.style.Styler, col: str, scale_colors: list[str]):
        ...
        def _fmt(val, idx):
            ...
            color = scale_colors[int(bins.iloc[idx])]; return f"background-color:{color};color:#111111"
    ...
    by_ag = (df.groupby(["TRECHO","AGENCIA_NORM"], as_index=False)
    ...
    # ✅ FIX: use st.write para renderizar Styler (st.dataframe trava)
    st.write(sty1)
...
def tab3_top3_precos(df_raw: pd.DataFrame):
    ...
    • Para CADA (Trecho, ADVP_CANON), pega a ÚLTIMA DATAHORA_BUSCA.
    • Mostra Top1/2/3 da pesquisa mais recente daquele par.
    """
    ...
    last = (df.sort_values("DATAHORA_BUSCA")
              .groupby(["TRECHO","ADVP_CANON"], as_index=False)
              .tail(1)[["TRECHO","ADVP_CANON","IDPESQUISA"]])
    base = df.merge(last, on=["TRECHO","ADVP_CANON","IDPESQUISA"], how="inner")

    # top3 por par
    g = (base.groupby(["TRECHO","ADVP_CANON","AGENCIA_NORM"], as_index=False)
               .agg(PRECO_MIN=("PRECO","min")))
    def _top3(gr):
        gr = gr.sort_values("PRECO_MIN").reset_index(drop=True)
        out = {"Trecho": gr["TRECHO"].iat[0], "ADVP": gr["ADVP_CANON"].iat[0]}
        for i in range(3):
            out[f"Top{i+1} Agência"] = gr["AGENCIA_NORM"].iat[i] if i < len(gr) else "-"
            out[f"Top{i+1} Preço"]   = gr["PRECO_MIN"].iat[i]      if i < len(gr) else np.nan
        return pd.Series(out)
    tabela = g.groupby(["TRECHO","ADVP_CANON"]).apply(_top3).reset_index(drop=True)
    for c in [c for c in tabela.columns if "Preço" in c]:
        tabela[c] = pd.to_numeric(tabela[c], errors="coerce").round(0).astype("Int64")
...
def tab4_ranking_agencias(df_raw: pd.DataFrame):
    ...
    wins = wins.sort_values("Top1 Wins", ascending=False).head(15)
    st.altair_chart(make_bar(wins, "Top1 Wins", "AGENCIA_NORM"), use_container_width=True)
