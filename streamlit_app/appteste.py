...
def tab3_top3_precos(df_raw: pd.DataFrame):
    ...
    # Renderizar todo o bloco HTML de uma só vez para evitar múltiplas chamadas dinâmicas a st.markdown
    out_html = []
    for trecho in trechos_sorted:
        ...
        if boxes:
            out_html.append(f"<div style='{TRE_HDR_STYLE}'>Trecho: <b>{trecho}</b></div>")
            out_html.append("<div style='" + GRID_STYLE + "'>" + "".join(boxes) + "</div>")
    if out_html:
