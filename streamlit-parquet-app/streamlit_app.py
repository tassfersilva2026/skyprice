...
def load_base(path: Path) -> pd.DataFrame:
    ...
    colmap = {
        0:"IDPESQUISA", 1:"CIA", 2:"HORA_BUSCA", 3:"HORA_PARTIDA", 4:"HORA_CHEGADA", 5:"TIPO_VOO",
        6:"DATA_EMBARQUE", 7:"DATAHORA_BUSCA", 8:"AGENCIA_COMP", 9:"PRECO", 10:"TRECHO", 11:"ADVP", 12:"RANKING"
    }
...
def make_bar(df: pd.DataFrame, x_col: str, y_col: str, sort_y_desc: bool = True):
    ...
    return (alt.Chart(d).mark_bar().encode(
        ...
        tooltip=[alt.Tooltip(f"{y_col}:N", title=y_col), alt.Tooltip(f"{x_col}:Q", title=x_col)],
...
def make_line(df: pd.DataFrame, x_col: str, y_col: str, color: str | None = None):
    ...
    enc = dict(x=x_enc, y=alt.Y(f"{y_col}:Q", title=y_col), tooltip=[alt.Tooltip(f"{x_col}", title=x_col), alt.Tooltip(f"{y_col}:Q", title=y_col)])
    if color:
...
def render_filters(df_raw: pd.DataFrame, key_prefix: str = "flt"):
    ...
    with c1:
        dt_ini = st.date_input("Data inicial (col. H)", key=f"{key_prefix}_dtini", value=st.session_state["flt"]["dt_ini"], min_value=dmin_abs, max_value=dmax_abs, format="DD/MM/YYYY")
    with c2:
        dt_fim = st.date_input("Data final (col. H)", key=f"{key_prefix}_dtfim", value=st.session_state["flt"]["dt_fim"], min_value=dmin_abs, max_value=dmax_abs, format="DD/MM/YYYY")
    with c3:
        advp_sel = st.multiselect("ADVP (col. L)", [1,5,11,17,30], default=st.session_state["flt"]["advp"], key=f"{key_prefix}_advp")
    with c4:
        trechos_all = sorted([t for t in df_raw["TRECHO"].dropna().unique().tolist() if str(t).strip()!=""])
        tr_sel = st.multiselect("Trechos (col. K)", trechos_all, default=st.session_state["flt"]["trechos"], key=f"{key_prefix}_trechos")
    with c5:
        hh_sel = st.multiselect("Hora da busca HH (col. C)", list(range(24)), default=st.session_state["flt"]["hh"], key=f"{key_prefix}_hh")
    ...
    mask &= (pd.to_datetime(df_raw["DATAHORA_BUSCA"], errors="coerce").dt.date >= dt_ini)
    mask &= (pd.to_datetime(df_raw["DATAHORA_BUSCA"], errors="coerce").dt.date <= dt_fim)
    if advp_sel:
...
def tab1_painel(df_raw: pd.DataFrame):
    ...
    Wg = W.replace({"R1":{"MAXMILHAS":"GRUPO 123","123MILHAS":"GRUPO 123"}, "R2":{"MAXMILHAS":"GRUPO 123","123MILHAS":"GRUPO 123"}, "R3":{"MAXMILHAS":"GRUPO 123","123MILHAS":"GRUPO 123"}})
    ...
    # 1. Apresentar GRUPO 123
    bloco_percent("GRUPO 123", Wg, "GRUPO 123")
    st.markdown("<hr>", unsafe_allow_html=True)

    # 2. Apresentar as outras N agências com mais vitórias em 1º lugar
    if not W.empty:
        # Contar vitórias em 1º lugar
        top_agencies = W["R1"].value_counts().index.tolist()
        
        # Remover agências que já estão no GRUPO 123 e "SEM OFERTAS"
        agencies_to_show = [
            agency for agency in top_agencies 
            if agency not in ["123MILHAS", "MAXMILHAS", "GRUPO 123", "SEM OFERTAS"]
        ]
        
        # Exibir as próximas 4 agências (ou menos, se não houver tantas)
        for agency in agencies_to_show[:4]:
...
def tab3_top3_precos(df_raw: pd.DataFrame):
    ...
    for i, (_,row) in enumerate(t.iterrows()):
        with cols[i]:
            st.metric(f"{row['AGENCIA_NORM']} • {row['TRECHO']}", value=f"R$ {row['PRECO']:,.2f}".replace(",", "X").replace(".", ",").replace("X","."), delta=f"{row['DATAHORA_BUSCA']:%d/%m/%Y}")
...
def tab10_exportar(df_raw: pd.DataFrame):
    ...
    st.download_button(
        ...
        data=csv_bytes,
        file_name="OFERTAS_filtrado.csv",
        mime="text/csv"
    )
