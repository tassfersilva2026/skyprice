...
def render_filters(df_raw: pd.DataFrame, key_prefix: str = "flt"):
    ...
    with c3:
        # Implementando seleção de ADVP igual à configuração de trechos
        advp_all = sorted([1,5,11,17,30])
        advp_sel = st.multiselect("ADVP (col. L)", advp_all,
...
def tab1_painel(df_raw: pd.DataFrame):
    ...
    def bloco_percent_todas_agencias(title: str, base: pd.DataFrame):
        """Mostra percentuais para todas as agências principais"""
        st.markdown(f"### {title}")
        
        # Identifica as agências com mais vitórias em 1º lugar
        top_agencias = base["R1"].value_counts().head(10).index.tolist()
        
        if not top_agencias:
            st.info("Nenhuma agência encontrada nos dados filtrados")
            return
        
        # Cria blocos para cada agência principal
        for agencia in top_agencias:
            if agencia == "SEM OFERTAS":
                continue
                
            p1 = (base["R1"] == agencia).mean()*100
            p2 = (base["R2"] == agencia).mean()*100
            p3 = (base["R3"] == agencia).mean()*100
            ...
            for i,(lab,val) in enumerate([("1º",p1),("2º",p2),("3º",p3)]):
                with (c1 if i==0 else c2 if i==1 else c3):
                    ...
                    st.progress(int(round(val)) if val <= 100 else 100)
            
            st.markdown("---")
    
    # Chama a função que mostra todas as agências
    bloco_percent_todas_agencias("Desempenho por Agências", W)
...
def tab2_top3_agencias(df_raw: pd.DataFrame):
    ...
    for i, (_, row) in enumerate(t3.iterrows()):
        ...
        with cols[i]:
            ...
            st.progress(int(round(pct)) if pct <= 100 else 100)
...
def main():
    ...
    with t[0]:
        try:
            ...
            st.error("Erro na aba Painel")
            st.exception(e)
    
    with t[1]:
        try:
            tab2_top3_agencias(df_raw)
        except Exception as e:
            st.error("Erro na aba Top 3 Agências")
            st.exception(e)
    
    with t[2]:
        try:
            tab3_top3_precos(df_raw)
        except Exception as e:
            st.error("Erro na aba Top 3 Preços")
            st.exception(e)
    
    with t[3]:
        try:
            tab4_ranking_agencias(df_raw)
        except Exception as e:
            st.error("Erro na aba Ranking por Agências")
            st.exception(e)
    
    with t[4]:
        try:
            tab5_preco_periodo(df_raw)
        except Exception as e:
            st.error("Erro na aba Preço por Período do Dia")
            st.exception(e)
    
    with t[5]:
        try:
            tab6_buscas_vs_ofertas(df_raw)
        except Exception as e:
            st.error("Erro na aba Qtde de Buscas x Ofertas")
            st.exception(e)
    
    with t[6]:
        try:
            tab7_comportamento_cias(df_raw)
        except Exception as e:
            st.error("Erro na aba Comportamento Cias")
            st.exception(e)
    
    with t[7]:
        try:
            tab8_competitividade(df_raw)
        except Exception as e:
            st.error("Erro na aba Competitividade")
            st.exception(e)
    
    with t[8]:
        try:
            tab9_melhor_preco_diario(df_raw)
        except Exception as e:
