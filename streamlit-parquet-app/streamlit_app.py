...
def _init_filter_state(df_raw: pd.DataFrame):
    ...
    st.session_state["flt"] = {
        "dt_ini": dmin,
        "dt_fim": dmax,
        # ADVP dinâmico (igual Trechos): nada pré-selecionado
        "advp": [],
        "trechos": [],
        "hh": [],
    }
...
def tab1_painel(df_raw: pd.DataFrame):
    ...
    # Base de vencedores (preenche SEM OFERTAS quando não há posição)
    W  = winners_by_position(df)
    # Versão com GRUPO 123 agregado
    Wg = W.replace({
    ...
    # --- LISTA COMPLETA DE ALVOS (MESMO ZERADOS) ---
    # pega todas as agências que existem na base inteira (não só no filtro),
    # remove duplicatas e organiza; depois injeta GRUPO 123 e SEM OFERTAS.
    agencias_all = sorted(set(df_raw["AGENCIA_NORM"].dropna().astype(str)))
    show_grp123  = ("MAXMILHAS" in agencias_all) or ("123MILHAS" in agencias_all)

    # não mostrar as individuais quando agregamos pelo grupo
    base_targets = [a for a in agencias_all if a not in ("MAXMILHAS","123MILHAS")]
    # SEM OFERTAS vai no fim (para aparecer inclusive % no 2º e 3º)
    base_targets = [a for a in base_targets if a != "SEM OFERTAS"]

    targets = (["GRUPO 123"] if show_grp123 else []) + base_targets + ["SEM OFERTAS"]

    def bloco_percent(title: str, base: pd.DataFrame):
        p1 = float((base["R1"] == title).mean())*100
        p2 = float((base["R2"] == title).mean())*100
        p3 = float((base["R3"] == title).mean())*100
        st.markdown(f"### {title}")
        ...
        for i,(lab,val) in enumerate([("1º",p1),("2º",p2),("3º",p3)]):
            with (c1 if i==0 else c2 if i==1 else c3):
                ...
                st.progress(int(round(max(0,min(val,100)))))  # garante 0–100

    # Render vertical: uma agência por bloco, inclusive zeradas e SEM OFERTAS
    for tgt in targets:
        base = Wg if tgt == "GRUPO 123" else W
        bloco_percent(tgt, base)
        st.markdown("<hr style='opacity:.1;margin:10px 0'>", unsafe_allow_html=True)
